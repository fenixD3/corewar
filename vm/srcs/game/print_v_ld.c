...
void		ldi_print_rest(t_corewar *corewar, t_parse_args *arg_val,
														const char *cmd)
{
	int		val_addr_1;
	int		val_addr_2;
															...
															const char *cmd)
{
	...
	((val_addr_1 + val_addr_2) % IDX_MOD < 0 && corewar->carriages->op_pos +
		...
		: do_steps(corewar->carriages->op_pos, (val_addr_1 + val_addr_2)
		% IDX_MOD, corewar->arena) - corewar->arena);
	ldi_print_rest(corewar, arg_val, cmd);
}
...
void		lldi_print_rest(t_corewar *corewar, t_parse_args *arg_val,
							const char *cmd)
{
	int		val_addr_1;
	int		val_addr_2;
	...
	fprintf(file, "P%5d | %s %d %d r%d\n       | -> load from"
		"%d + %d = %d (with pc %ld)\n",
			corewar->carriages->id, cmd, val_addr_1, val_addr_2,
			arg_val->val[2], val_addr_1, val_addr_2, val_addr_1 + val_addr_2,
														...
														const char *cmd)
{
	...
	ft_printf("P%5d | %s %d %d r%d\n"
		...
		: do_steps(corewar->carriages->op_pos, (val_addr_1 + val_addr_2),
		corewar->arena) - corewar->arena);
	lldi_print_rest(corewar, arg_val, cmd);
}
