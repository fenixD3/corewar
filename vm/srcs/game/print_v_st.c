...
void	sti_print_rest(t_corewar *corewar, t_parse_args *arg_val,
													const char *cmd)
{
	int				val_addr_1;
	int				val_addr_2;
	...
	fprintf(file,
		"P%5d | %s r%d %d %d\n       | -> store to %d + %d = %d"
		"(with pc and mod %ld)\n",
		corewar->carriages->id, cmd, arg_val->val[0], val_addr_1,
		val_addr_2, val_addr_1, val_addr_2, val_addr_1 + val_addr_2,
													...
													const char *cmd)
{
	...
	: do_steps(corewar->carriages->op_pos, (val_addr_1 + val_addr_2) %
		IDX_MOD, corewar->arena) - corewar->arena);
	sti_print_rest(corewar, arg_val, cmd);
}
