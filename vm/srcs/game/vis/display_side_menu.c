...
void			draw_frame(int player, int i, int extndd)
{
	SDL_Color	color;
	SDL_Rect	rectangle;
	int			rec_h;
...
void			display_side_menu(t_corewar *crwr, int ind)
{
	int			i;
	t_vc		*vc;
	int 		done;
	...
	while (vc != NULL && i < 900)
	{
		...
		|| (ind >= 0 && vc->carriage->op_pos == &(crwr->arena[ind])))
		{
			if (!done && (ind - 4100 > i && ((vc->is_open == true) ?
				...
				vc->is_open = (vc->is_open == true) ? (done = 1) == 0 : true;
			draw_frame(vc->carriage->reg[0], i, vc->is_open);
